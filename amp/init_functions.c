	...
	for (k = 0; k < K; ++k)
	{
		...
		for (t = 0; t < T; ++t)
		{
			if(gsl_matrix_get(pilot,k,t) == 1)
			{
				...
				gsl_matrix_complex_set(x_h_pre,k,t,gsl_matrix_complex_get(x,k,t));
				gsl_matrix_complex_set(x_b,k,t,gsl_matrix_complex_get(x,k,t));
			}
			else 
			{
				gsl_matrix_complex_set(x_h,k,t,def);
				gsl_matrix_complex_set(x_h_pre,k,t,def);
...
void init(double sn)
{
	...
	x_h_pre = gsl_matrix_complex_calloc(K,T);
	...
	h_h_pre = gsl_matrix_complex_calloc(N,K);
	h_h_abs2 = gsl_matrix_calloc(N,K);
	eta = gsl_matrix_calloc(N,K);
	eta_b = gsl_matrix_calloc(N,K);
	h_b = gsl_matrix_complex_calloc(N,K);
