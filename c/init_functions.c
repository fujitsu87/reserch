...
void init_x()
{
	...
	for (k = 0; k < K; ++k)
	{
		for (t = 0; t < T; ++t)
		{
			...
			{
				GSL_SET_COMPLEX(&z,a_k*gsl_matrix_get(re_pilot,k,t-(T-Tp)),a_k*gsl_matrix_get(im_pilot,k,t-(T-Tp)));
	...
	// PrintRealMatrix(stdout,size,size,re_pilot);
