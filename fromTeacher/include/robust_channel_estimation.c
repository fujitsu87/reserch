...
double Computation_zeta_c_robust(complex **Z)
{
  int i,m,t;
  double zeta,a_i;
  gsl_vector *a;

  a = gsl_vector_alloc(M*T);

  for(i=0,m=0;m<M;m++) {
    for(t=0;t<T;t++,i++) {
      a_i = cabs(Z[m][t]);
      //printf("%d %d %g %g %g\n",m,t,creal(Z[m][t]),cimag(Z[m][t]),a_i);
      gsl_vector_set(a,i,a_i);
    }
  }

  zeta = QMedian(M*T,a)/gsl_cdf_ugaussian_Pinv(0.75);
  zeta *= zeta;

  a = GSLRealVectorFree(a);
			...
			complex **H,double *mse)
{
  ...
  for(i=0;i<iter;i++) {
    for(m=0;m<M;m++) {
      for(t=0;t<T;t++) {
	Z_c[m][t] = Computation_z(m,t,Y,H_es,Vh,X_es,Vx,Z_c,zeta_c,Z_d,zeta_d,
				  X_es_pre,H_es0);
	//printf("%d %d %g %g\n",m,t,creal(Z_c[m][t]),cimag(Z_c[m][t]));
      }
    }
    for(t=0;t<T;t++) {
      zeta = Computation_zeta_c_robust(Z_c);

      for(m=0;m<M;m++) {
	zeta_c[m][t] = zeta;
	//zeta_c[m][t] = Computation_zeta(m,t,H_es,Vh,X_es,Vx);
	//printf("%d %d %g %g\n",m,t,zeta,Computation_zeta(m,t,H_es,Vh,X_es,Vx));
      }
    }
    //printf("\n");
