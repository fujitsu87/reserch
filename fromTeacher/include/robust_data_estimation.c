...
double Computation_zeta_d_robust(int t,complex **Z)
{
  int m;
  double zeta,a_m;
  gsl_vector *a;
  ...
  for(m=0;m<M;m++) {
    ...
    gsl_vector_set(a,m,a_m);
  }

  zeta = QMedian(M,a)/gsl_cdf_ugaussian_Pinv(0.75);
  zeta *= zeta;
		     ...
		     complex **Z_c,double **zeta_c,complex **X,double *ber)
{
  ...
  for(i=0;i<iter;i++) {
    ...
    zeta = Computation_zeta_d_robust(t,Z_d);
    for(m=0;m<M;m++) {
      zeta_d[m][t] = zeta; 
      //if(m==0) printf("%g %g\n",zeta,Computation_zeta(m,t,H_es,Vh,X_es,Vx));
    }

    for(n=0;n<N;n++) {
      //Vx[n][t] = zeta;
      //Vx[n][t] = sigma2 + N*Vx[n][t]/(double)M;
      Vx[n][t] = Computation_Vx(n,t,H_es,zeta_d);
      X_es[n][t] = Computation_X_es(n,t,H_es,Vh,X_es,Vx,Z_d,zeta_d,Z_c,zeta_c,X_es_pre);
