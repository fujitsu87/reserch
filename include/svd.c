...
void SVDLeft(gsl_matrix_complex *A, double sigma[], gsl_matrix_complex *U)
{
	...
	//出力生成
	{
		...
		for(i = 0; i < _AAH_->size1; i++)
		{
			...
			if(lambda < 0)
			{
				gsl_vector_complex_view Ucol = gsl_matrix_complex_column(U, i);
				gsl_complex z = {-1.0, 0.0};
				gsl_vector_complex_scale(&Ucol.vector, z);	//ベクトル符号修正
				sigma[i] = 0;								//特異値0
			}else{
...
void SVDRight(gsl_matrix_complex *A, double sigma[], gsl_matrix_complex *V)
{
	...
	//出力生成
	{
		...
		for(i = 0; i < _AHA_->size1; i++)
		{
			...
			if(lambda < 0)
			{
				gsl_vector_complex_view Vcol = gsl_matrix_complex_column(V, i);
				gsl_complex z = {-1.0, 0.0};
				gsl_vector_complex_scale(&Vcol.vector, z);	//ベクトル符号修正
				sigma[i] = 0;								//特異値0
			}else{
				sigma[i] = sqrt(lambda);					//平方根代入
			}
