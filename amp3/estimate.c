    ...
    for (k = 0; k < ENSEMBLE; ++k)
    {
        ...
        for (i = 0; i < BIG_LOOP; ++i)
        {
            ...
            for (j = 0; j < H_LOOP; ++j)
            {
                ...
                mse_h_n[i] += culc_complex_mse(N,K,h,h_h)/(double)ENSEMBLE;
                mse_h_n_my[i] += culc_complex_mse_etc(N,K/DK,h,h_h,0)/(double)ENSEMBLE;
                mse_h_n_other[i] += culc_complex_mse_etc(N,K,h,h_h,K/DK)/(double)ENSEMBLE;
                abs2_matrix(h_h,h_h_abs2,N,K);
